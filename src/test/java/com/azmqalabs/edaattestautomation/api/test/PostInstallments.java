...
public class PostInstallments {
	
	static String TestDataTab = "InstallmentsPOST";
	...
	ExtentTest test;
	Log Log;
	InstallmentsMethods InstallmentsMethods;

	@Factory(dataProvider = "TestDataProvider")
	public PostInstallments(Map<Object, Object> testdatamap) {
	...
	public void testpostDiscount() throws Exception {
		try {
			InstallmentsMethods.Installment(testdatamap);		
			test.log(Status.PASS, "Installment Has been Created");
	...
	public void setup() throws Exception {
		...
		test = ExtentReportManager.CreateExtentReportTest(extent, className, testdatamap.get("TestDescription").toString(),
				TestScriptID);
		InstallmentsMethods= new InstallmentsMethods(test);
		Log =new Log(test);
