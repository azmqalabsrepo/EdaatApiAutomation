...
//	Log Log;
	...
	public BillsMethods(ExtentTest test) {
		this.test = test;

	}

	public void Bills(Map<Object, Object> testdatamap) {
		try {
			String URI = commonMethods.launchapiUrl();
			...
			case "GETWITHBILLNO":
				...
				response = given().headers("Authorization", "bearer " + commonMethods.generateToken(URI))
						.contentType(ContentType.JSON).pathParam("billNo", testdatamap.get("BillNo").toString())
